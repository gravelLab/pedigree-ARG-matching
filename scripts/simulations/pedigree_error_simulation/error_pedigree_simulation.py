...
def run_interactive_session():
    ...
    initial_pedigree = PloidPedigree.get_ploid_pedigree_from_file(filepath=pedigree_path)
    # List of non-founder individuals
    non_founder_vertices = list({x // 2 for x in initial_pedigree if initial_pedigree.has_parents(x)})
    ...
    try:
        for i in range(simulation_steps):
            ...
            for index, vertex in enumerate(random_individuals):
                ...
                vertex_parents = {y for x in vertex_ploids for y in initial_pedigree.get_parents(x)}
                ...
                random_parent_level = initial_pedigree.get_vertex_level(2 * random_parent)
                # Find the ploid id of the vertex that is connected to this parent
                random_parent_children_ploids = [x for x in vertex_ploids
                                                 if initial_pedigree.get_parents(x) == random_parent_ploids]
                ...
                initial_pedigree_levels = initial_pedigree.get_levels()
                max_attempts = len(initial_pedigree_levels[random_parent_level]) * 2
                attempts = 0
                while attempts < max_attempts:
                    new_parent_ploid = random.choice(initial_pedigree_levels[random_parent_level])
                    if new_parent_ploid not in vertex_parents:
                        break
                    attempts += 1
                else:
                    new_parent_ploid_candidates = [x for x in initial_pedigree_levels[random_parent_level] if x
                ...
                initial_pedigree.remove_edge(parent=2 * random_parent, child=child_ploid)
                initial_pedigree.remove_edge(parent=2 * random_parent + 1, child=child_ploid)
                initial_pedigree.add_edge(parent=2 * new_parent, child=child_ploid)
                initial_pedigree.add_edge(parent=2 * new_parent + 1, child=child_ploid)
            ...
            for parent, child in add_edges:
                initial_pedigree.add_edge(parent=parent, child=child)
            for parent, child in remove_edges:
                initial_pedigree.remove_edge(parent=parent, child=child)
