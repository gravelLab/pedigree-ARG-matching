...
def random_subselect_poisson(input_list, percentage):
    n = len(input_list)
    simulated_errors = numpy.random.poisson(percentage, n)
    selected_individuals = [input_list[index] for index in range(n) if simulated_errors[index] > 0]
    # Check that all the selected individuals are in the input list
    assert not [x for x in selected_individuals if x not in input_list]
...
total_distance_from_incorrect_solution = 0
...
def log_incorrect_solutions(alignments: list[dict], simulation_step: int):
...
print(f"Clades: {len(valid_alignments_dict)}")
...
average_distance_to_identity = None
identity_solution = None
if len(valid_alignments_dict) == 1:
    # Supporting multiple clades requires a bit more complicated logic, so it's skipped for now
    identity_solution = {x: x for x in set(coalescent_tree.children_map).union(coalescent_tree.probands)}
    assert identity_solution in valid_alignments
    average_distance_to_identity = sum(
    ...
    general_result_file.write(f"Average distance to the identity is: {average_distance_to_identity}\n")
...
try:
    for i in range(simulation_steps):
        ...
        graph_matcher = GraphMather(coalescent_tree=coalescent_tree, processed_graph=potential_mrca_graph, logger=logger)
        result = graph_matcher.find_mapping()
        # Find the incorrect solutions
        # assert len(result) == len(valid_alignments_dict)
        ...
        log_incorrect_solutions(incorrect_alignments, i)
        general_result_file.write("#####################################\n")
        general_result_file.write(f"Simulation step: {i}\n")
        general_result_file.write(f"Number of incorrect alignments in this simulation {len(incorrect_alignments)}\n")
        ...
        if len(valid_alignments_dict) == 1 and len(incorrect_alignments) > 0:
            total_distance_from_incorrect_solution += sum(
                get_alignments_proband_distance_probands_ignored(alignment, identity_solution, coalescent_tree.probands)
                for alignment in incorrect_alignments
            )
            general_result_file.write(f"New average distance from incorrect solutions to the"
                                      f" identity is: "
                                      f"{total_distance_from_incorrect_solution / total_number_of_incorrect_solutions}\n")
        general_result_file.write("#####################################\n")
        general_result_file.flush()
...
if len(valid_alignments_dict) == 1:
    incorrect_alignments_to_identity_average_distance = (total_distance_from_incorrect_solution
                                                         / total_number_of_incorrect_solutions)
    general_result_file.write(f"Initial average distance to the identity is: {average_distance_to_identity}\n")
    general_result_file.write(f"Average distance from incorrect solutions to the identity "
                              f"is: {incorrect_alignments_to_identity_average_distance}\n")
    general_result_file.write(f"Average distance delta is: "
                              f"{incorrect_alignments_to_identity_average_distance - average_distance_to_identity}\n")
